...
class NMT(nn.Module):
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y_ts = torch.split(Y, split_size_or_sections=Y.size(0), dim=0)[0]
        for Y_t in Y_ts:
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        alpha_t = torch.unsqueeze(F.softmax(e_t, dim=1), dim=2)
        a_t = torch.squeeze(torch.bmm(enc_hiddens.permute(0,2,1), alpha_t), dim=2)
        dec_hidden
